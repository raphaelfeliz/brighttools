...
- Set desktop `column-gap` to 45px on `.product` wrapper.
...
## Tasks
- [x] 01. Update `.product` style rules in `mar26-main-product.liquid` under the desktop media query (`min-width: 1000px`) to set `column-gap: 45px !important;` and `row-gap: 0.875rem !important;` (instead of a single `gap: 0.875rem !important;`). Bump section version to 1.7.0. [agent: 2703a6a2-1341-44fd-a732-82cec0707bca] [blocks: 02] [blocked_by: none]
...
- [x] 03. Verify in desktop view that the gap between the image gallery and the product details column is visually increased to 45px, and that mobile remains unaffected. [agent: Raphael] [blocks: 04] [blocked_by: 02]
